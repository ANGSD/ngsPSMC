...
double smartsize1(int tk_l,int numWind,double **fw,double **bw,double *tk,double *P1,double **emis,double pix,double rho,double *newEpSize){
  ...
  for(int i=0;i<remap_l;i++){
	...
	if (j == 0)
	  tt[i] = tk[at-1];
	//	fprintf(stderr,"\t\tepsize[%d]:%f stationary[%d]:%f W[%d]:%f tt[%d]:%f tk[%d]:%f\n", at-1, epsize[at-1], at-1, stationary[at-1], i, W[i], i, tt[i], at-1, tk[at-1]);
    }
    tW += W[i];
  }
  
  for(int i=0;i<remap_l - 1;i++){
    newEpSize[i] = -log(1 - W[i]/tW)/(tk[i+1]-tk[i]);
    tW -= W[i];
  }
  
  double noRec = 0;
  for(int w = 1; w < numWind; w++){
    noRec += fw[remap_l-1][w]*P1[remap_l-1]*emis[w+1][remap_l-1]*bw[remap_l-1][w+1];
  }
  noRec = noRec / numWind / pix;
  newEpSize[remap_l - 1] = (exp(-2*rho*tk[remap_l-1])/noRec-1)/(2*rho);
}
